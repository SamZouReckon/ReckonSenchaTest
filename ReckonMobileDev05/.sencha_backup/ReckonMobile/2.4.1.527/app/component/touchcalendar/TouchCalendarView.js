 ...
 */
/**
 * @class Ext.ux.touchcalendar.TouchCalendarView
 * @author Stuart Ashworth
 *
 * For use with Sencha Touch 2
 *
 */
Ext.define('RM.component.touchcalendar.TouchCalendarView', {
    ...
    config: {
        ...
        baseTpl: [
        '<table class="{[this.me.getViewMode().toLowerCase()]}" id="touchcalendertbl">',
            '<thead>',
                '<tr id="touchcalendernav">',
                    '<th class="{[this.me.getPrevPeriodCls()]}"></th>',
                    '<th></th>',
                    '<th colspan="3"><span>{[Ext.Date.format(this.me.currentDate, "F")]} {[Ext.Date.format(this.me.currentDate, "Y")]}</span></th>',
                    '<th></th>',
                    '<th class="{[this.me.getNextPeriodCls()]}"></th>',
                '</tr>',
            '</thead>',
        '</table>',
        '<table id="slideCalender" style="display:{[this.getCollapsable()]}">',
            '<thead>',
                '<tr>',
                ...
                '</tr>',
            '</thead>',
            '<tbody id="touchcalenderbody">',
                '<tr class="time-block-row">',
                    '<tpl for=".">',
						'<td class="time-block {[this.getClasses(values)]}" datetime="{[this.me.getDateAttribute(values.date)]}" height="32px">',
							'{date:date("j")}',
						'</td>',
		...
		'<table>',
			'<tr>',
				'<td style="border: none; background-color: #4e4e4e;">',
                    '<div style="margin: 12px; border: none"></div>',
                    '<div align="center" class="rm-swipebar"></div><div align="center" class="rm-swipebar"></div><div align="center" class="rm-swipebar"></div>',
                    '<div style="margin: 8px; border: none"></div>',
				'</td>',
			'</tr>',
        '</table>',
    ...
    commonTemplateFunctions: {
        ...
        getHeaderClass: function(currentIndex){
            //return currentIndex === 2 ? this.me.getPrevPeriodCls() : currentIndex === 6 ? this.me.getNextPeriodCls() : '';
            return currentIndex === 1 ? this.me.getPrevPeriodCls() : currentIndex === 7 ? this.me.getNextPeriodCls() : '';
        },
        
        
        getCollapsable: function(currentIndex){
    ...
    constructor: function(config){
        ...
        /**
             ...
             * @param {Ext.ux.touchcalendar.TouchCalendarView} this
    ...
    applyViewMode: function(viewMode){
        ...
        var viewModeFns = RM.component.touchcalendar.TouchCalendarView[viewMode.toUpperCase()];
        ...
        Ext.apply(this.commonTemplateFunctions, {
            me: this
        })
        
        new Ext.XTemplate((viewModeFns.tpl || this.getBaseTpl()).join(''), this.commonTemplateFunctions)
        
        var calTpl = new Ext.XTemplate((viewModeFns.tpl || this.getBaseTpl()).join(''), this.commonTemplateFunctions)
        //            new Ext.XTemplate(
        //            '<tpl if="leaf">',
    ...
    statics: {
        ...
        MONTH: {
            ...
            getStartDate: function(date){
                return Ext.bind(RM.component.touchcalendar.TouchCalendarView.WEEK.getStartDate, this)(new Date(date.getFullYear(), date.getMonth(), 1));
            },
