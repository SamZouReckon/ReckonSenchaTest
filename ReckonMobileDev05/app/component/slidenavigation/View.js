 ...
 */
Ext.define('RM.component.slidenavigation.View', {
    ...
    onSelect: function(list, item, eOpts) {
        ...
        if (Ext.isFunction(this._cache[index])) {
            this._cache[index]();
        } else if(item.get('activated')) { //Added by MB
            container.setActiveItem(this._cache[index]);
        }
        else{
            me.list.select(4, false); //Added by MB - Select the Dashboard for now if current item is not activated
            Ext.Msg.alert('Not Activated', item.get('title') + ' is not activated.', Ext.emptyFn);
        }
    ...
    createContainer: function() {
        return Ext.create('Ext.Container', Ext.merge({}, this.config.container, {
            ...
            layout: 'card',
            // Added by MB - will detect if user taps on right hand panel (except toolbar) and close it
            listeners: {
                tap: {
                    element: 'element',
                    fn: function (e) {
